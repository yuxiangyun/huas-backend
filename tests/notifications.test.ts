 ...
 * [OUTPUT]: 覆盖 recipient eventId、Outbox 幂等/撤销/退避、actor 投影、逐条已读权限、未读计数与已读清理
 * [POS]: tests 的 Notifications 专项回归，锁定 UGC 集成前可独立验证的事务事件与用户读模型边界
...
describe('Notifications user read model', () => {
  ...
  test('cleanup removes expired read notifications but preserves unread and recent read facts', async () => {
    const now = new Date('2026-07-31T12:00:00.000Z');
    const events = [
      createLikeEvent(firstActorUserId),
      ...createActivityEvents({
        actorUserId: secondActorUserId,
        recipientUserIds: [recipientUserId],
        type: 'discover_comment',
    ...
    await db.update(schema.notifications).set({
      readAt: new Date(now.getTime() - DEFAULT_NOTIFICATIONS_POLICY.readRetentionMs - 1),
    }).where(eq(schema.notifications.id, rows[0]!.id));
    await db.update(schema.notifications).set({ readAt: new Date(now.getTime() - 1_000) })
      .where(eq(schema.notifications.id, rows[1]!.id));

    expect(await notifications.cleanup.run(now)).toBe(1);
    const remaining = await db.select().from(schema.notifications);
    expect(remaining).toHaveLength(2);
    expect(remaining.some((row) => row.readAt === null)).toBe(true);
    expect(remaining.some((row) => row.readAt !== null)).toBe(true);
  });
});
