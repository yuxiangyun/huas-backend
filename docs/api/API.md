...
| `GET /calendar/schedule.ics` | `studentId + sig` 查询参数 | 当前学期完整课表 ICS 订阅 |
...
| 错误码 | 含义 | HTTP 状态码 |
|---|---|---:|
| `3001` | CAS 登录明确拒绝或登录参数不可用 | 400 |
| `3002` | 验证码错误或需要验证码 | 400 |
| `3003` | 学校明确拒绝保存凭据或要求验证码，需要整体重新登录 | 401 |
| `3004` | 学校上游超时 | 504 |
| `4001` | JWT 无效或过期 | 401 |
| `4002` | 参数错误 | 400 |
...
- `GET /calendar/schedule.ics` 返回智慧文理移动教务当前学期的全部课程，保留每次课程的真实日期。
- 首次有效订阅采集当前学期各周，全部验证成功后保存完整 ICS。上游逐周读取是一轮采集，不是单个 HTTP 请求；共用学校会话恢复与 45 秒总预算。
- 订阅使用独立的 `calendar-semester:{userId}` SQLite 快照，每用户保存最近完整 ICS 和上次采集开始时间，不拼接普通周缓存。
- 只有距上次采集开始满 24 小时后的有效订阅请求才能触发下一轮。窗口内任意次数订阅均返回同一快照，读取不延长窗口；并发请求合并为一轮。
- 登录、生成订阅链接、普通课表查询、普通课表强制刷新及其他业务均不消耗、重置或受此窗口限制；订阅没有绕过窗口的强刷参数。
- 采集开始前持久化机会，服务重启与采集失败均不重置窗口。中途失败保留上次完整 ICS；没有完整快照则报错，后续窗口内返回 `3005/503` 并提供 `data.nextRetryAt`，不返回残缺或伪造空日历。
- 这是订阅请求驱动的更新，没有每日后台全员拉取任务。日历软件下次抓取时才能看到已更新内容；ICS 的 `P1D` 刷新提示不强制客户端准时抓取。
...
| 用户名或密码错误 | 3001 | 400 |
| 学校超时 | 3004 | 504 |
| CAS 成功，Portal/JW 尚未获取或不可用 | 成功 | 200 |
| CAS 普通 403/429、缺 execution 或响应无法识别 | 3005 | 503 |

补充说明：

- 验证码会话十分钟有效，到期即拒绝并一次消费，不依赖后台清理；学校页面或验证码初始化故障不计为密码失败。
- CAS 明确验证身份成功即完成本服务登录，不等待 Portal/JW 激活或资料回填；姓名或班级缺失时，服务端发出不阻塞响应的尽力资料补全。
- 成功响应中的 `data` 仅包含 `token` 与 `user`，不再返回额外能力字段。
- 首次姓名可缺省、班级可为空，成功响应仍保留 token、user 对象与 studentId；学校能力按需获取，缺失资料由后台补全。
- Portal、JW 和两个 mobile 能力独立按需恢复；读取 JW 不要求 Portal 可用，读取 mobile 不激活 JW。
- CAS 成功但 Portal/JW 均不可用时仍签发 JWT；后续学校业务返回 3005/503 或 3004/504，不触发退出。并发真实登录均可成功，迟到结果不能覆盖较新的成功认证状态。
- 成绩回源在真实 CAS 成功后仍无法激活 JW 时返回 `3005/503`（明确超时保持 `3004/504`），可按既有非凭据错误规则回退旧成绩缓存；不会将学校服务故障转换为重新登录循环。
...
返回当前用户当前学期全部课程的订阅 ICS，URL 与签名规则保持不变。
...
- 包含当前学期全部教学周，学期范围和课程日期以移动教务响应为准；不包含其他历史学期
- 节次时间由服务端本地静态映射转换为具体时分
- 若签名错误，返回 `4001/401`
- 若用户不存在，返回 `4001/401`
- 全部周均有效且确实无课时可返回空学期日历；未公布、缺少元信息或部分周失败均不能当作完整空课表
- 固定使用移动教务单源，不受后台课表来源策略影响，也不回退 JW 或 Portal 课表
- 使用独立学期快照，仅有效订阅请求可在上次采集开始满 24 小时后回源，其他行为不参与此限制；详见 3.5
- 任何采集失败均保留最近完整订阅快照及本轮机会；无完整快照时报错，避免错误响应清除客户端已有课程
...
- `/auth/login` 经学校认证或本地快捷成功后，只要本地姓名或班级缺失，就会发出一次不阻塞响应的资料补全；失败仅记录日志，后续登录仍可重试
- `/api/user` 回源成功时按代次更新 `users.name/className`；命中资料缓存时也会补齐空字段，但不会用旧缓存覆盖已有值
