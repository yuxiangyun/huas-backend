...
- 收到 `3003`：学校侧凭证恢复失败，也需要引导用户重新登录；明确 CAS 拒绝已在后端阻断本地快捷登录，不会再次用保存的旧密码直接放行。
...
| 用户名或密码错误 | 3001 | 400 |
| 学校超时 | 3004 | 504 |
| CAS 成功但 Portal/JW 均不可用 | 3005 | 503 |
| CAS 响应无法识别 | 3005 | 503 |
...
- CAS 成功但 Portal/JW 均不可用时，返回 `3005/503`，不签发 JWT，不累计密码失败次数；真实 CAS 上下文仍按既有事务提交。
