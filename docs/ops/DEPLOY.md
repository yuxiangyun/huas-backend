...
PM2 槽位配置以 `interpreter: none` 直接执行 `bun run src/index.ts`，并在目标槽停写后删除旧进程元数据再按 ecosystem 重建。不要改回 `startOrReload` 或 PM2 的 Bun interpreter：前者不会收敛已存在进程的 script path，后者会通过 CommonJS `require()` 装载入口，二者都无法可靠承载当前含顶层异步初始化的 ESM 进程入口。
