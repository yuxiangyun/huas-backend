...
discover/: Discover 移动端信息流页面，以共享 Social 字标、紧凑发布动作与懒任务外壳组合移动优先全屏详情和评论 widgets
login/: 普通用户校园身份登录页面
...
treehole/: Treehole 移动端连续白色双形态信息流页面，以共享 Social 字标和懒任务外壳组合图文发布、移动优先全屏详情、社区资料、分享与评论 widgets
...
树洞、好饭使用统一 PageHeader 顶部节奏与 SocialPageTitle 字标，消息与我的不展示页级文字标题；树洞和好饭的移动发布动作共享与品牌字标重量匹配的尺寸、线宽与垂直留白；消息页无 `tab` 默认展示互动，私信分区以 `tab=conversations` 显式表达；Treehole 分享统一生成 `postId` canonical 深链，帖子详情与公共资料保持 URL 互斥，私信路由只保存目标 userId。
