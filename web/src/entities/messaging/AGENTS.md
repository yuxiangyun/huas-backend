...
api/messaging-queries.ts: TanStack Query 查询与 mutation 编排，保持消息历史三态游标及会话缓存失效
...
架构决策
空会话不在客户端伪造：目标入口先读取 CommunityProfile，首条消息成功后才采用服务端 conversationId。
会话列表普通分页只服务人工加载，增量刷新使用 lastMessageId 高水位；消息历史保持 latest/before/after 语义，不用 offset 模拟实时消息。
图文发送使用浏览器 UUID 作为 Idempotency-Key，失败后的相同草稿复用原 UUID；私有媒体由共享原语按 URL 去重并转为引用计数 Blob URL。
