...
成员清单
api/discover-api.ts: 根据认证事实把元数据/Feed/详情/评论读取切到 `/api/public/discover` 或 `/api/discover`，本人内容与全部写操作固定使用 Bearer 路由
...
匿名读取只消费服务端公共作者三字段投影，`likedByMe/isMine` 固定为 false；token 变化由 Provider 清空 Query，防止匿名快照与登录后的 viewer 个性化投影共用缓存事实。
