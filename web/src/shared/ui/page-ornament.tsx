...
export function PageOrnament({
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden border border-white/78 shadow-[0_22px_48px_rgba(15,23,42,0.09)] backdrop-blur-[24px]',
    ...
    >
      <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.42),rgba(255,255,255,0.02))]" />
      <div className="absolute -right-7 -top-7 size-24 rounded-full bg-white/58 blur-2xl" />
      <div
        className={cn(
          'absolute -left-7 bottom-0 size-20 rounded-full blur-3xl',
          ornamentGlowClasses[tone]
        )}
      />
      <div className="absolute bottom-3 right-3 h-7 w-12 rounded-full border border-white/62 sm:bottom-4 sm:right-4 sm:h-8 sm:w-14" />
      <div className="absolute bottom-6 right-7 size-2 rounded-full bg-white/88 sm:bottom-7 sm:right-8" />
        ...
        {badges.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {badges.slice(0, compact ? 2 : 3).map((badge) => (
              <span
                key={badge.label}
                className="inline-flex min-w-0 max-w-full items-center gap-1.5 rounded-pill bg-white/68 px-2.5 py-1.5 text-[0.72rem] font-medium text-muted ring-1 ring-white/66"
              >
                <IconBubble
                  className="size-5 rounded-[0.7rem] text-[0.72rem] shadow-none"
                  icon={badge.icon}
                  size="sm"
