...
普通用户 UI 采用纯白页面基底与 shadcn new-york + neutral 语言：四个主 Tab 通过 shared/ui 的本地中文楷体字标统一标题与顶部节奏；Treehole 有图帖使用 Instagram 首图层级但正文不重复作者标识，纯文字帖将正文独占排列在作者栏下方；Treehole 与好饭统一采用图标右侧弱化数字的计数互动栏，好饭使用首图沉浸式媒体流，聊天气泡限制内容宽度。
Treehole 私有图片以 Bearer Blob 按 URL/认证模式进入 10 分钟、24MB 会话内 LRU：首页只挂载接近视口的每帖首图，详情/全屏只挂载当前和相邻图片；聊天图片接近视口才请求，token 变化立即清空缓存。
...
2026-08-01: Treehole 前端接入 multipart 图文闭环，有图帖采用 Instagram 首图信息流、文字帖采用 X 阅读布局，补齐动态上传限制、失败保稿、详情滑动/全屏、分享、个人内容缩略图与后台图片审核。
