...
普通用户 UI 采用纯白页面基底与 shadcn new-york + neutral 语言：四个主 Tab 通过 shared/ui 的品牌图片字标或本地中文楷体字标统一标题与顶部节奏，Treehole 与好饭发布动作统一为黑色正圆按钮内居中白色加号；Treehole 有图帖使用 Instagram 首图层级但正文不重复作者标识，纯文字帖将正文独占排列在作者栏下方；Treehole 与好饭统一采用图标右侧弱化数字的计数互动栏，详情互动栏只保留正文与动作之间的顶分隔线；两类帖子详情以移动优先全屏单列阅读，桌面端再扩展为媒体与评论双栏，聊天气泡限制内容宽度。
