...
架构决策
utils 默认无状态、无业务副作用；request-body-limit.ts 统一上传解析前门禁但由调用方提供业务容量与错误文案；image.ts 只统一图片解码与转换，Discover、Community、Messaging 各自保留存储、权限和生命周期 adapter；discover.ts 只为旧调用方再导出 modules/discover/domain，不再保存领域规则副本。
