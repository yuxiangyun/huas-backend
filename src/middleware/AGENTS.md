...
logging.middleware.ts: 请求日志中间件，收集耗时、身份、响应元信息和 HTTP 细节；Social 聚合未读、Notifications/Messaging 普通读取与稳定增量 GET 仅在成功时静默访问日志
...
轮询 quiet 模式只作用于成功 GET 的文件/控制台访问日志；4xx/5xx、写操作及 app 工厂中的 HTTP metrics 不得被跳过。
