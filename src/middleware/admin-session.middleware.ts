 ...
 * [POS]: middleware 的后台独立认证边界，以 HttpOnly Cookie 替代浏览器持久化 Basic 凭证
...
function pruneSessions(now: number) {
  ...
  const oldestToken = [...sessions.entries()].sort((left, right) => left[1].touchedAt - right[1].touchedAt)[0]?.[0];
...
export function currentAdminSession(c: Context) {
  ...
  return resolved ? { username: resolved.session.username, expiresInSeconds: Math.floor(IDLE_TTL_MS / 1000) } : null;
}
