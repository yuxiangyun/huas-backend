...
migrations/: 不可变编号 migration 目录，保存 baseline、社交 contract、Community/Treehole/Early Rising 的前向结构演进
repair.ts: Discover/Treehole 派生计数显式修复内核，支持无写入 dry-run 与幂等事务更新
schema.ts: 全局 Drizzle 类型相，声明身份凭证/学校登录上下文、Early Rising 打卡与展示设置、Treehole 私有图片、通知与 Messaging 等全部 SQLite 结构映射
...
Early Rising 的每日打卡与单行展示设置均属于 SQLite 事实，设置默认显示个人资料入口并保存最后管理员审计字段。
...
2026-08-24: 补记 0005 Community Bio、0006 Early Rising 打卡，并新增 0007 Early Rising 展示设置单行快照。
