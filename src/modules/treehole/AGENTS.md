...
开发规范
点赞幂等且允许作者自赞、评论计数与删除清理必须保留 SQLite 事务边界；自我点赞不生成通知；删帖/删评论（含管理端）在同事务经 Outbox 撤回对应活动事件与已投影通知，发帖媒体补偿只覆盖写库失败、投影失败不得回收已入库媒体。
Operations 管理列表经 TreeholeOperationsQueryPort 读取内容事实与公共作者，不暴露校园敏感身份。

变更日志
2026-08-16: createPost 收敛为仅 INSERT 返回 ID，响应投影外提到补偿边界之外；删除路径同事务撤回 Outbox 事件与通知。
