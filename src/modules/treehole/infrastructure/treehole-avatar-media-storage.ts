/**
 * [INPUT]: 依赖 sharp、Node 文件系统、Bun.file、config 头像路径与注入的 TreeholePersistence 可见性校验
 * [OUTPUT]: 对外提供 TreeholeAvatarMediaStorage 头像压缩/存储/删除/公开读取能力与缓存头常量
 * [POS]: modules/treehole/infrastructure 的本地头像媒体 adapter，文件格式与缓存语义的唯一实现
...
export class TreeholeAvatarMediaStorage implements TreeholeAvatarStorage {
  constructor(private readonly persistence: TreeholePersistence) {}
  ...
  async getPublicFile(requestPath: string): Promise<ReturnType<typeof Bun.file> | null> {
    ...
    const publicPath = `${config.treehole.avatarMediaBasePath}/${fileName}`;
    if (!(await this.persistence.isPublishedAvatar(userId, publicPath))) return null;
  ...
  private fileNameFromUserId(userId: number) {
    return `${userId}.webp`;
  }

  private resolveFileNameFromRequestPath(requestPath: string) {
  ...
  private async readSourceMetadata(source: Buffer): Promise<Metadata> {
    try {
