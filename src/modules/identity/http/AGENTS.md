...
auth.routes.ts: Identity 登录 Hono 适配器，负责 JSON/限流/日志与错误映射；未知 CAS 响应及认证后服务不可用返回既有 3005/503，登录参数保持兼容，验证码维护由 SchoolAccess 向根周期注册器提供
...
学校 execution/验证码读取和初始化异常返回 3005/503，明确超时仍为 3004/504；只有用户输入或明确认证拒绝适用认证失败语义。
