...
sqlite-identity.store.ts: IdentityStorePort 的 SQLite/Drizzle 实现，在用户 upsert 事务内调用共享 commitRealSchoolLoginContext，避免与静默恢复复制 epoch、基础凭证和派生清理逻辑
...
SQLite store 独占用户登录写事务；共享 SchoolLoginContext 原语在该事务内完成交互标记删除、CAS/Portal/JW upsert、缺失 Portal 删除、epoch 推进与严格派生清理，杜绝部分身份事实和跨 epoch Portal JWT 可见。
