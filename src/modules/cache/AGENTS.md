...
infrastructure/: SQLite 缓存读写、过期判定、兼容解码与 LRU 裁剪
cache-service.ts: Cache composition root，装配 SQLite store、进程内 singleflight 与可注入低基数观察器，并保留稳定静态 API

架构决策
Cache 模块只显式表达现有本地 SQLite 语义：`ttlMs: 0` 永不自动过期，版本不兼容只产生 miss；业务模块决定何时刷新与如何 stale fallback。
