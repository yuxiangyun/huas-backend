 ...
 * [POS]: mobile-yxt 的认证交换边界；仅接受 Portal JWT，不接受调用方 CookieJar，tid/refreshToken 只存在于单次调用内，Cookie 白名单不在此重复
...
export interface MobileYxtSessionExchangePort {
  exchange(portalJwt: string, deadlineAt?: number): Promise<MobileYxtSessionExchange>;
}
...
export class MobileYxtAuthExchanger implements MobileYxtSessionExchangePort {
  async exchange(portalJwt: string, deadlineAt?: number): Promise<MobileYxtSessionExchange> {
    // 权限隔离点：使用独立 Jar，不带入 CAS TGC 或 Portal Cookie。
    ...
    const openResponse = await mobileClient.request(openUrl.toString());
