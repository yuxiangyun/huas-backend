/**
 * [INPUT]: 依赖统一 AppError/ErrorCode 与 mobile-yxt 传输、认证、业务和协议失败事实
 * [OUTPUT]: 对外提供无敏感正文的类型化错误构造、凭证拒绝判定、低敏感 operation/stage、HTTP 状态校验、传输错误归一化与 stale/fatal 判定
...
export function normalizeMobileYxtTransportError(error: unknown): MobileYxtError {
  ...
  if (message === 'REQUEST_TIMEOUT') return mobileYxtTimeout();
  if (/ECONNRESET|EAI_AGAIN|ETIMEDOUT|ENOTFOUND|fetch failed|network/i.test(message)) {
    return mobileYxtUnavailable();
  }
