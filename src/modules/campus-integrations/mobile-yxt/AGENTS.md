...
mobile-yxt-errors.ts: 认证拒绝及其类型判定、超时、5xx、业务拒绝和协议漂移的无敏感正文语义；协议错误携带不进入公开响应的 operation/stage，并标注 stale 资格；传输归一化只归 SchoolAccess
...
ecard-overview-service.ts: 通过 Portal 余额服务与 SchoolAccess 交易操作聚合，使用固定长度用户月键、每用户 6 条 LRU、独立配额、同键 miss/refresh 合流及子源级 availability/freshness
...
架构决策
恢复与重试归 SchoolAccess；本目录保留会话持久化、单次交换、纯解析、分页与业务缓存，不再拥有独立 executor。共享恢复结果只有冻结字符串/数字快照，业务调用各自保留 20 秒预算及客户端；请求前或分页/串行步骤之间的统一超时均保持既有 stale 资格。
...
交易只允许当前月及此前 23 个自然月，缓存每用户最多保留 6 个热月份；同月 cache miss 与 refresh 共享在途回源，overview 分别投影余额/交易 freshness 与 staleParts；未知结构和协议错误不写缓存、不 stale，只有 CAS 交互要求返回 3003，超时固定 3004。
refundFlag 原样投影；当前官方 H5 为字符串 0/1 提供普通/退款展示证据，但在真实样本证明退款会计规则前，totals 仅表示上游有符号金额的机械分类求和，不宣称退款已正确冲正。
...
派生会话拒绝仅用于内部重建；恢复后二次拒绝或竞争耗尽向外返回 503，只有基础恢复的明确 CAS 交互要求才返回 3003。
