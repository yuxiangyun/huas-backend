...
恢复失败按学校登录 epoch 绑定固定五秒窗口，CAS 按用户、Portal/JW 按能力隔离，命中不续期；真实登录换代使旧窗口失效，迟到静默请求不能覆盖新登录或补写交互认证标记；明确 CAS 凭据拒绝与验证码共用无 TTL 交互标记阻断本地快捷登录，CAS 成功后学校能力不足返回 3005/503，未知 CAS 响应不伪装密码错误。本地快捷登录不检查学校也不清冷却。Portal HTTP 5xx 作为无 token 的附带故障证据返回：真实登录仍尝试 JW，恢复路径传播异常并冷却，不改变 JWT 放行条件。静默重认证按用户共享在途恢复（PerKeySingleflight），同用户并发凭证缺口只跑一条 CAS 登录链；共享结果携带实际能力，joiner 在航班结束后优先以新 TGC 串行补足所需系统。upstream 经 resolveCredentialClient 单次恢复链同时取得 token 与客户端，禁止恢复链跑两遍；业务 session expired 只按请求所用凭证快照与登录代次条件失效，迟到响应不得误删新登录/普通轮换的当前凭证。学校 HTTP 的单次与总预算覆盖完整正文读取。
