...
sqlite-notification-repository.ts: recipient 隔离的 createdAt/id 排序分页、单调 ID 增量、未读/总量摘要与逐条幂等已读 adapter
...
通知列表由 `(recipient_user_id, created_at DESC, id DESC)` 直接索引承载；增量按主键 ID 升序读取，摘要单次聚合 total/unreadCount，已读事实永久保留。
