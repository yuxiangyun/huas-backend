...
成员清单
application/: 通知普通翻页/ID 增量、未读/逐条已读用例与 Outbox 可重试投影
domain/: 六类活动事件、评论差异接收规则、稳定 eventId/增量 DTO 与依赖倒置 ports
http/: 注入式活动通知普通列表、增量轮询、未读计数与单条已读 Hono 路由
...
普通列表 offset 只服务人工翻页；轮询必须按 notification ID 高水位调用增量入口，避免插入造成重复或漏项。已读通知不自动清理，也不引入归档。
