...
互动事实与 activity_outbox 由 UGC adapter 在同一 SQLite 事务提交；eventId 按 recipient 幂等，unlike 按 eventId、删帖按 resource、删评论按 subresource 在同事务撤回未投影事件与已投影通知，请求后即时投影与周期重试复用同一 projector。
