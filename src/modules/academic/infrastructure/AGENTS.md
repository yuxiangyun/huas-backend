...
成员清单
cache-store.ts: Academic 缓存读写/快照条件失效/保时无覆盖提升、singleflight 与 refresh stale fallback 适配入口，复用 canonical Cache 语义
runtime.ts: 默认 SchoolAccess 具名读取/cache/fallback 端口实现集合，仅供 composition root 装配
