...
mobile-jw-semester-service.ts: 当前学期完整采集，复用一次当前周锚点，45 秒共享预算内串行读取其余周，严格核对日期/学期/总周数，失败不返回部分课程；不读写普通周缓存
schedule-facade.ts: 移动教务/JW/Portal 编排，以 50 秒请求截止时间约束策略读取、按剩余来源分配 current 等待并预留 2 秒 stale 仲裁，不取消 reader 共享回源；用户首选只前置 current 并去重，stale 仍按后台原参与范围/固定顺序读取；另提供日历固定移动教务单源入口，固化请求级快照元信息排除来源能力限制参与最终错误仲裁，并保留 legacy 主源未公布短路与错误优先级；明确无数据在穷尽来源及 stale 后返回中文初始化/发布提示，不作为成功课表缓存
