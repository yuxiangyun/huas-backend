...
admin.routes.ts: createAdminRoutes 注入式后台路由工厂，承载 dashboard、公告、受限 multipart 三态底栏首页弹窗、Early Rising 展示设置、社区/Treehole 帖子媒体、Messaging 会话增量/三态只读历史审计与 Academic 策略协议
...
public.routes.ts: `/api/public/announcements` 与 `/api/public/index-popup` 匿名内容读取 HTTP 适配器
