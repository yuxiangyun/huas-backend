...
admin.routes.ts: createAdminRoutes 注入式后台路由工厂，承载 dashboard、analytics、公告、日志、社区、Messaging 只读与 Academic 课表来源策略管理协议
...
跨领域运维命令只调用根组合注入的公开应用端口；Messaging 管理面只有受 Cookie 会话保护的 GET 会话/消息/私有媒体读取，课表策略管理不得直接写 Academic 状态文件。
