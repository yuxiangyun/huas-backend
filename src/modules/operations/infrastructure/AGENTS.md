...
架构决策
本目录只实现 Operations 自有持久化与运行态 adapters；首页弹窗配置/媒体跟随 dirname(DB_PATH) 跨蓝绿槽共享，配置原子替换且公开媒体有界保留最近三个版本，消除配置切换期旧 URL 的 404 竞态；跨领域命令由根组合按公开 port 注入，不在此目录持有业务 concrete。
