 ...
 * [POS]: modules/discover/infrastructure 的媒体 adapter，负责图片文件副作用与公开读取可见性
...
export class DiscoverMediaService implements DiscoverMediaStorage {
  storeImages(files: File[]) {
    return DiscoverMediaService.compressAndStoreImages(files);
  }

  removeStorage(storageKey: string) {
    return DiscoverMediaService.removeStorage(storageKey);
  }
