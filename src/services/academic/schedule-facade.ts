/**
 * [INPUT]: 依赖 ScheduleService、PortalScheduleService、resolveFallbackError 与日期/错误工具
 * [OUTPUT]: 对外提供 ScheduleFacade、ScheduleFacadeResult 与 ScheduleRequestMeta
 * [POS]: services/academic 的课表编排门面，统一 JW/Portal 优先级、日期范围、fallback 与响应元信息
...
type ScheduleSource = 'jw' | 'portal';
type ScheduleCacheState = 'hit' | 'miss' | 'bypass' | 'fallback';
type ScheduleLookup = 'weekly' | 'legacy' | 'range';
...
export class ScheduleFacade {
  static async getJwFirstSchedule(options: {
    userId: number;
    studentId: string;
    date?: string;
    ...
    try {
      ...
      try {
        const result = await PortalScheduleService.getSchedule(
          options.userId,
          options.studentId,
          range.startDate,
          range.endDate,
  ...
  static async getPortalFirstSchedule(options: {
    userId: number;
    studentId: string;
    startDate?: string;
    endDate?: string;
