...
export class AdminDashboardService {
  static async getDashboard(query: DashboardQuery) {
    ...
    const [
      ...
      cacheRows,
      credentialRows,
      totalDiscoverPostRows,
      totalDiscoverRatingRows,
      majorRows,
      gradeRows,
      discoverPostRows,
      announcements,
    ...
    return {
      ...
      metrics: {
        ...
        totalDiscoverPosts: Number(totalDiscoverPostRows[0]?.count || 0),
        totalDiscoverRatings: Number(totalDiscoverRatingRows[0]?.count || 0),
        memory: {
      ...
      discover: {
        totalPosts: Number(totalDiscoverPostRows[0]?.count || 0),
        totalRatings: Number(totalDiscoverRatingRows[0]?.count || 0),
        items: discoverPostRows.map((row) => ({
          id: row.id,
          title: row.title || '',
